# -*- coding: utf-8 -*-
"""
Created on Mon Oct 17 14:23:43 2016
...
for W_ind in range(nR1):
    ...
    W = newton(disp_rel, W, tol=1e-5, maxiter=50)
    print('W = ', W)
    ...
    for t_ind in range(nt):
        ...
        ax.axes.y_label = ''
        ax.axes.z_label = ''
        mlab.savefig(u'D:/my_work/visualisations/asym_rho_fixed/around avoided crossing/f' + '%.6f' % R1 + '_' + str(t_ind+1) + '.png')
        mlab.close()
        t = t + (t_end - t_start)/nt
    R1 = R1 + (R1_end - R1_start)/nR1
