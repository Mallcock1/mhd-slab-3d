...
for t_ind in range(nt):
    ...
    if show_disp_top == True:
        xixvals_t = np.real(np.repeat(sf.xix_amd(xvals, zvals, t, W)[:, :, np.newaxis], ny, axis=2))
        xizvals_t = np.real(np.repeat(sf.xiz_amd(xvals, zvals, t, W)[:, :, np.newaxis], ny, axis=2))
        xiyvals_t = np.real(np.repeat(sf.xiy_amd(xvals, zvals, t, W, vA_func)[:, :, np.newaxis], ny, axis=2))
    
    if show_vel_top == True:
        vxvals_t = np.real(np.repeat(sf.vx_amd(xvals, zvals, t)[:, :, np.newaxis], ny, axis=2))
        vzvals_t = np.real(np.repeat(sf.vz_amd(xvals, zvals, t)[:, :, np.newaxis], ny, axis=2))
    ...
    bzvals_t = np.real(np.repeat(-sf.bz_amd(xvals, zvals, t, W, vA_func)[:, :, np.newaxis], ny, axis=2) +
                       bz_eq3d)
